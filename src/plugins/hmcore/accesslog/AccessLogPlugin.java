 ...
 */
package plugins.hmcore.accesslog;
...
{
   //host remoteAddress - requestUser [time] "requestUrl" status bytes "referrer" "userAgent" requestTime "POST"
	/**
	 * custom config
	 * accesslog.log2play=true
	 * accesslog.logpost=true
	 * accesslog.path=logs/access.log
	 * accesslog.log2db=true
	 */
   private static final String FORMAT = "[AccessLog] %mt %v %h - %u [%t] \"%r\" %s %b \"%ref\" \"%ua\" %rt \"%post\"";
   ...
   public void onConfigurationRead()
   {
      ...
      _shouldLogPost = Boolean.parseBoolean(Play.configuration.getProperty(CONFIG_PREFIX+"logpost", "true"));
   ...
   private synchronized void log()
   {
      ...
      String line = FORMAT;
      line = StringUtils.replaceOnce(line, "%mt", request.method);
...
//         String body = request.params.get("body");
         String body = Json.toJson(request.params.data);
